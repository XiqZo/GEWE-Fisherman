...
function FishingScene:fightingFishUpdate(
    ...
    local reelSpeed =
        math.abs(change * acceleratedChange)

    -- Determine reel speed feedback
    if reelSpeed < self.safeMinReelSpeed then

        self.reelFeedback = "TOO SLOW"

    elseif reelSpeed > self.safeMaxReelSpeed then

        self.reelFeedback = "TOO FAST"

    else

        self.reelFeedback = "PERFECT"

    end

    -- Normal reel / escape logic
...
function FishingScene:drawOverlay()
    ...
    elseif self.state == STATES.fightingFish then
        ...
        -- Reel speed feedback in center of screen
        self:drawReelFeedback()
...
function FishingScene:drawReelFeedback()

    local text = self.reelFeedback
    ...
    local y = 120 - textHeight / 2

    -- White background makes the message easier to see
    gfx.setColor(gfx.kColorWhite)
